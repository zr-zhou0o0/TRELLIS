...
def init_nodes(save_depth=False, save_normal=False, save_albedo=False, save_mist=False, save_mask=False):
    ...
    if save_mask:
        ...
        # Store object information instead of references to avoid StructRNA issues after scene normalization
        mask_object_info = []
        for i, obj in enumerate(mask_objects):
            obj.pass_index = i + 1
            max_passidx = max(max_passidx, i + 1)
            mask_object_info.append({
                'name': obj.name,
        ...
        if mask_objects:
            ...
            spec_nodes['mask_object_info'] = mask_object_info
...
def main(arg):
    ...
    for i, view in enumerate(views):
        ...
        if arg.save_mask:
            if 'mask_object_info' in spec_nodes:
                print(f"[DEBUG] Mask object info after normalization: {spec_nodes['mask_object_info']}")
                for obj_info in spec_nodes['mask_object_info']:
                    if obj_info['name'] in bpy.data.objects:
                        obj = bpy.data.objects[obj_info['name']]
                        print(f"[DEBUG] Object {obj_info['name']}: exists, pass_index = {obj.pass_index}")
                    else:
                        print(f"[WARNING] Object {obj_info['name']} not found after normalization")
